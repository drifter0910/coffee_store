...
const ShopDetail: FC<Props> = ({products, addToCart}) => {
  const [quan, setQuan] = useState<any>(1);
  const { productId } = useParams();
  var item = products?.find((item: any) => {
                ...
                <div className="shopdetail-r-wrapper">
                  ...
                  <div className="shop-detail-r-quan">
                    ...
                    <input
                      ...
                      type="number"
                      value={quan}
                      onChange={(e) => setQuan(parseInt(e.target.value))}
                    />
            ...
            <div className="author-r">
              ...
              <div className="autour-cmt">
                <p>
                  ...
                  probatus ad. Quo eu etiam exerci dolore, usu ne omnes referrentur. Ex eam diceret
                  denique, ut legimus similique vix detraxit
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="shopdetail-related">
          <Row gutter={[16, 16]}>
            {relateddata.map((item: any,index) => (
              <Col key={index} xl={6} md={6} sm={6} xs={12}>
                <div
...
const mapDispatchToProps = (
  dispatch: (arg0: { type: string; payload: { id: number; qty: number } }) => any
) => {
  return {
    addToCart: (id: number, qty: number) => dispatch(addToCart(id, qty)),
  };
};
